...
<!--
  Two flavours of this file; the badges are the only thing that differs. GitHub
  renders .github/README.md in preference to this one, GitLab renders only this
  one, so each forge shows its own pipeline and opens the coverage report its own
  pipeline published. Everything between the markers is generated: edit
  ci/readme-sync.sh, run it, and it rewrites both files. `--check` in both
