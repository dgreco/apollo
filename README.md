...
| **Work** | `/plan <task>`, `/init [notes]` (write AGENTS.md), `/diff [args]`, `/loop <prompt> [--times N] [--every S]`, `/bg <prompt>` (background session), `/agents` · `/tasks`, `/stop [id]`, `/review [focus]` (independent subagent review), `/goal [text\|show\|clear]` (standing objective), `/queue [prompt\|clear]`, `/moa <prompt>` (mixture-of-agents), `/learn <what>` (capture a skill), `/worktree [list\|new [name]\|prune]`, `/snapshot [create\|list\|restore <id>\|prune]`, `/rollback [list\|create\|<number>]` (git working-tree checkpoints) |
