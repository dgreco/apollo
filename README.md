...
| | |
|---|---|
| **Session** | `/status` · `/history` (model, message count, token usage, context %), `/usage`, `/config`, `/profile`, `/reset` · `/new`, `/clear`, `/redraw`, `/title <name>`, `/compress` · `/compact` (force compaction), `/save [file.md]`, `/retry`, `/copy`, `/image <path>` (attach to next message), `/sessions`, `/resume <id\|latest>`, `/branch` · `/fork [name]` |
| **Model** | `/model [name]`, `/reasoning <level>`, `/reasoning-display`, `/verbose`, `/version` · `/v`, `/whoami` |
| **Work** | `/plan <task>`, `/init [notes]` (write AGENTS.md), `/diff [args]`, `/loop <prompt> [--times N] [--every S]`, `/bg <prompt>` (background session), `/agents` · `/tasks`, `/stop [id]`, `/review [focus]` (independent subagent review), `/worktree [list\|new [name]\|prune]`, `/snapshot [create\|list\|restore <id>\|prune]` |
